...
public class PersonController extends EM {
    ...
    public List<Person> getAllCustomers() {
        ...
        TypedQuery<Person> query = EntityManagerHandler.INSTANCE.getEntityManager()
                .createQuery("SELECT p FROM Person p WHERE p.customer = true " +
                                "ORDER BY p.identification ASC",
                        Person.class);
    ...
    public List<Person> getAll() {
        open();
        TypedQuery<Person> query = EntityManagerHandler.INSTANCE.getEntityManager()
                .createQuery("SELECT p FROM Person p ORDER BY p.identification ASC",
                        Person.class);
    ...
    public List<Person> searchCustomer(String str) {
        ...
        TypedQuery<Person> query = EntityManagerHandler.INSTANCE.getEntityManager()
                .createQuery(
                        "SELECT p FROM Person p WHERE p.typePerson = 'Cliente' AND p" +
                                ".identification like '%" + str + "%' OR REPLACE(p.identification" +
                                ", '.', '') like '%" + str + "%' OR p.acronym like '%" + str +
                                "%' OR p.firstName like '%" + str + "%' OR p.lastName like '%" +
                                str + "%' ORDER BY p.identification ASC",
                        Person.class);
        return sort(query.getResultList());
    }
    ...
    public List<Person> sort(List<Person> people) {
        Collections.sort(people, new Comparator<Person>() {
            public int compare(Person o1, Person o2) {
                long result = (extractLong(o1.getIdentification()) -
                        extractLong(o2.getIdentification()));
                if(result > 0) {
                    return 1;
                } else {
                    return -1;
