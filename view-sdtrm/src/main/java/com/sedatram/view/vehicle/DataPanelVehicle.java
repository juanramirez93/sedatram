...
public class DataPanelVehicle extends DataPanelAbstract<Vehicle> {
	...
	protected void defineFields() {
		...
		plaqueField.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent e) {
				...
				if (vehicle != null) {
					...
					data = new Vehicle(plaqueField.getText());
					fillData();
				}
			}
		});
		...
		AutoCompleteDecorator.decorate(typeField, TypeVehicleController.getInstance().getAll(), false,
		...
		AutoCompleteDecorator.decorate(brandField, BrandVehicleController.getInstance().getAll(), false,
				ObjectToStringConverter.DEFAULT_IMPLEMENTATION);
		...
		AutoCompleteDecorator.decorate(colorField, ColorVehicleController.getInstance().getAll(), false,
				ObjectToStringConverter.DEFAULT_IMPLEMENTATION);
	...
	public boolean saveData() {
		...
		if (error.isEmpty()) {
			...
			if (isNew) {
				int rta = JOptionPane.showConfirmDialog(null, StringsUtil.WISH_ADD_OWNER, StringsUtil.TITLE,
